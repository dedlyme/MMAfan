...
<section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 pb-10 sm:pb-16 px-4 sm:px-8">
    <div class="bg-white/90 dark:bg-gray-800/90 rounded-3xl shadow-xl p-6 sm:p-8 text-center backdrop-blur-md">
        ...
        <p class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-2">{{ auth()->user()->name }}</p>
    </div>
    <div class="bg-white/90 dark:bg-gray-800/90 rounded-3xl shadow-xl p-6 sm:p-8 text-center backdrop-blur-md">
        <div class="text-red-500 text-4xl sm:text-5xl mb-3">🥊</div>
...
{{-- ===== LIVE CHAT ===== --}}
<section class="bg-white/90 dark:bg-gray-900/90 rounded-3xl shadow-2xl border border-gray-200/30 dark:border-gray-700/40 p-4 sm:p-6 lg:p-8 mb-16 mx-3 sm:mx-8 backdrop-blur-md">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-3 sm:space-y-0">
        <h2 class="text-2xl sm:text-3xl font-extrabold text-red-500 drop-shadow-lg tracking-wide text-center sm:text-left">
            Live Chat
        </h2>
        <button onclick="document.documentElement.classList.toggle('dark')"
            class="px-3 sm:px-4 py-2 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition w-full sm:w-auto">
            Toggle Dark / Light
        </button>
    </div>
        ...
        <form id="chat-form" action="{{ route('messages.send') }}" method="POST"
            ...
            <input id="chat-input" type="text" name="message" placeholder="Type a message..."
                class="flex-1 p-3 rounded-xl border border-gray-300 dark:border-gray-600
                       ...
                       focus:ring-2 focus:ring-red-500 outline-none w-full transition-all duration-300"
                required>
            <button type="submit"
                class="px-5 py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition duration-300 w-full sm:w-auto">
                Send
            </button>
        </form>
    </div>
