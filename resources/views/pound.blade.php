    ...
    <div class="bg-gray-900/70 backdrop-blur-md rounded-3xl p-6 space-y-5 shadow-2xl">
                ...
                @if(auth()->check() && auth()->user()->is_admin)
                    <div class="flex space-x-3">
                        ...
                        <form action="{{ route('admin.pound.update', $fighter) }}" method="POST" class="flex space-x-2 items-center">
                            @csrf
                            @method('PATCH')
                            <input type="text" name="fighter_name" value="{{ $fighter->fighter_name }}" class="p-2 rounded bg-gray-700 text-white w-36 focus:ring-2 focus:ring-red-500 transition" required>
                            <input type="number" name="rank" value="{{ $fighter->rank }}" min="1" max="10" class="p-2 rounded bg-gray-700 text-white w-20 focus:ring-2 focus:ring-red-500 transition" required>
        ...
        @if(auth()->check() && auth()->user()->is_admin)
            <form action="{{ route('admin.pound.store') }}" method="POST" class="mt-8 flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-3">
                @csrf
                <input type="text" name="fighter_name" placeholder="New Fighter" class="p-3 rounded bg-gray-700 text-white flex-1 focus:ring-2 focus:ring-red-500 transition" required>
                <input type="number" name="rank" placeholder="Rank" min="1" max="10" class="p-3 rounded bg-gray-700 text-white w-28 focus:ring-2 focus:ring-red-500 transition" required>
